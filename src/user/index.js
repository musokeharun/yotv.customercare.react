...
export default class User extends Component {
  ...
  handleSubmit = async (e) => {
    ...
    if (number.startsWith("00000")) return await this.fetchNumber();
  ...
  reset = () => {
    ...
    Object.getOwnPropertyNames(state.data).forEach((prop) => {
      if (prop.toUpperCase() === "LIKELY") state.data[prop] = 0;
      else state.data[prop] = "";
    });
    this.setState(state);
  };

  fetchNumber = async () => {
    let vendor = this.state.responses["vendors"].find(
      (v) => v.title === this.state.vendor
    );
    try {
      let { data } = await Axios.post(
        baseUrl + "/user/call" + (vendor ? `?v=${vendor.id}` : "")
      );
  ...
  handleChange = (name, value, surface = false, alterState) => {
    ...
    if (alterState) state = alterState(state);

    // console.log(state);
    this.setState(state);
    let availability = state.data.availability;
    console.log(availability);
    if (availability.toUpperCase() !== "RECEIVED") this.otherFadeAway();
  ...
  onForward = async () => {
    ...
    const { id, forwardTo, number } = this.state;

    if (!id || !forwardTo || forwardTo === "" || number.startsWith("00000"))
      return;

    // alert(`${number} ${forwardTo} ${id}`);
    // return;

    try {
      let { data } = await Axios.post(baseUrl + "/user/forward", {
        call: id,
        user: forwardTo,
      });
      this.reset();
      ...
      console.log(e);
      toast("Could not forward number.", {
        type: "error",
      });
    } finally {
      stopLoad();
  ...
  render() {
    let { number, responses, others, data, vendor, category, forwardTo } =
      this.state;
    ...
    let currentCategory = responses["categories"]
      ? responses["categories"].find((c) => Number(c.id) === Number(category))
      : {};
          ...
          <form id={"appForm"} onSubmit={(e) => this.handleSubmit(e)}>
            <div className="content fs-6 d-flex flex-column flex-column-fluid">
              ...
              <div className={"card my-2"}>
                <div
                  className={
                    "row justify-content-center align-items-center rounded bg-transparent"
                  }
                >
                  {!!numberArray.length &&
                    numberArray.map((i) => (
                ...
                <div className={"mx-auto d-inline-block"}>
                  {!!others &&
                    Object.getOwnPropertyNames(others).map((p) => (
                      <span
                        key={v1()}
                        className={"badge bg-primary-soft float-left"}
                      >
                        {p}:
                        {typeof others[p] === "string" &&
                        others[p].includes("-")
                          ? new Date(others[p]).toDateString()
                          : others[p]}
                      </span>
                    ))}
                </div>
              ...
              <div id={"other"} className={"other w-100"}>
                {!!responsesLength && (
                  <div className={"card"}>
                    <div
                      className={
                        "row my-md-2 my-1 justify-content-between px-2"
                      }
                    >
                      <div className={"col-md-6 col-12 mb-3"}>
                        <div className="card bg-transparent shadow-none border-0 border-end">
                          <div className="card-header ">
                            <h4 className="card-header-title ">Gender</h4>
                          </div>
                      ...
                      <div className={"col-md-6 col-12 mb-3"}>
                        <div className="card bg-transparent shadow-none border-0">
                          <div className="card-header ">
                            <h4 className="card-header-title ">Likely</h4>
                          </div>
                      ...
                      <div className={"col-md-6  col-12 mb-3"}>
                        <div className="card bg-transparent shadow-none border-0">
                          <div className="card-header ">
                            <h4 className="card-header-title ">SubCategory</h4>
                          </div>
                          <div className="card-body border-0">
                            <Select
                              handleChange={({ currentTarget }) =>
                      ...
                      <div className={"col-md-6 col-lg-4 col-12 mb-3"}>
                        <div className="card bg-transparent shadow-none border-0">
                          <div className="card-header ">
                            <h4 className="card-header-title ">Resolution</h4>
                          </div>
                          <div className="card-body border-0 py-1">
                            <Input
                              name={"resolution"}
                      ...
                      <div className={"col-md-6 col-lg-4 col-12 mb-3"}>
                        <div className="card bg-transparent shadow-none border-0">
                          <div className="card-header ">
                            <h4 className="card-header-title ">Suggestion</h4>
                          </div>
                          <div className="card-body border-0 py-1">
                            <Input
                              name={"suggestion"}
                      ...
                      <div className={"col-md-6 col-lg-4 col-12 mb-3"}>
                        <div className="card bg-transparent shadow-none border-0">
                          <div className="card-header ">
                            <h4 className="card-header-title ">Other</h4>
                          </div>
                          <div className="card-body border-0 py-1">
                            <Input
                              name={"other"}
              ...
              <div className={"d-flex justify-content-center"}>
                ...
                >
                  <button
                    type={"submit"}
                    className="btn btn-outline-primary mx-4 btn-lg"
                  >
                    Submit &amp; Next
                  </button>
                  {!!responsesLength && (
                    ...
                    <div className="btn-group dropup" role="group">
                      ...
                      <div className="dropdown-menu">
                        <div className={"bg-transparent"}>
                          <div className={"card-body"}>
                            <div className={"flex-row mt-md-3 mt-1"}>
                              {responses["users"].map(({ id, email }, i) => (
                                <span title={email}>
                                  <Radio
                                    value={id}
                                    name={"forwardTo"}
                                    key={"forwardTo" + i}
                                    label={email.split("@")[0].toUpperCase()}
                                    currentValue={forwardTo}
                                    handleChange={(name, value) =>
                                      this.handleChange(name, value, true)
                                    }
                                  />
